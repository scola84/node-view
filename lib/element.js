...
class Element extends EventHandler {
  ...
  test() {
    ...
    if (this.tester) {
      test = Boolean(this.tester.value);
  ...
  setProperties(definition, set) {
    ...
    if (typeof properties === 'function' && set) {
  ...
  handleMatchAnimate(definition, context, matcher) {
    return this.setAnimation(scolaAssign({}, definition, {
      play: true
  ...
  setAnimation(definition, context, set) {
    ...
    definition = scolaAssign({}, definition);
  ...
  execute(method) {
    ...
    return this.setExecution(
      method,
      this.bindMatcher(
        this.handleMatchExecute,
        method
      )
    );
  }

  handleMatchExecute(method, matcher) {
    return this.setExecution(method, matcher.matches);
  }

  setExecution(method, set) {
    if (!set) {
      return this;
    }
  ...
  prepend(element) {
    this.node.insertBefore(element.get(), this.node.firstChild);
    element.emit('prepend', this);
